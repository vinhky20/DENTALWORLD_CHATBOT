...
def read_intent_report(path):
    ...
    jsonl = [{**v, 'config': path.parts[1]}
             for k, v in blob.items() if 'weighted avg' in k]
...
def read_pandas():
    ...
    intent_df = pd.concat([read_intent_report(p)
                          for p in paths if 'intent_report' in str(p)])
    paths = list(pathlib.Path("results").glob(
...
subset_df = intent_df.loc[lambda d: d['config'].isin(
    selected_config)].melt('config')
...
subset_df = entity_df.loc[lambda d: d['config'].isin(
    selected_config)].melt('config')
